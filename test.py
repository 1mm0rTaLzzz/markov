...
def draw_heatmap(p):
    ...
    cell_width = 1000 // cols
    cell_height = 800 // rows

    for i in range(rows):
        for j in range(cols):
            cell_value = p[i][j]
            color_intensity = int(255 - (cell_value * 25))
            ...
            if DISPLAY_PROBABILITY:
                if flag:
                    ...
                    pygame.draw.rect(screen, CYAN,
                                     pygame.Rect(201 + start_point[1] * square_size, 1 + start_point[0] * square_size,
                                                 square_size - 2,
                                                 square_size - 2), width=6)
                    pygame.draw.rect(screen, BROWN,
                                     pygame.Rect(201 + end_point[1] * square_size, 1 + end_point[0] * square_size,
                                                 square_size - 2,
...
def probability(world):
    ...
    start_point_str = textbox8.getText()
    end_point_str = textbox7.getText()

    # start_point = parse_string_to_arr(start_point_str)[0]
    # end_point = parse_string_to_arr(start_point_str)[0]
    ...
    robot_position = (start_point[0], start_point[1])
    ...
    return p, rows, cols, world, robot_position, end, prediction, repeat
...
def print_path(p, rows, cols, world, robot_position, end, prediction):
    # Найдем путь от начальной до конечной точки
    path = find_path(robot_position, end, world)
...
textbox7 = TextBox(screen, 130, 390, 60, 30, fontSize=20,
                   borderColour=(0, 0, 0), textColour=(0, 0, 0),
                   onSubmit=output, radius=1, borderThickness=1)

img = font.render('End Point (x,y): ', True, BLACK)
screen.blit(img, (10, 440))
textbox8 = TextBox(screen, 130, 440, 60, 30, fontSize=20,
                   borderColour=(0, 0, 0), textColour=(0, 0, 0),
                   onSubmit=output, radius=1, borderThickness=1)
    ...
    for event in events:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                print_path(p, rows, cols, world, robot_position, end, prediction)
            ...
            if event.button == 1:
                if 50 < pos[0] < 150 and 540 < pos[1] < 580:
                    p, rows, cols, world, robot_position, end, prediction, repeat = probability(world)
